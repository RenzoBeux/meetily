...
export const RecordingStatusBar: React.FC<RecordingStatusBarProps> = ({ isPaused = false }) => {
  ...
  return (
    <motion.div
      ...
      className="flex items-center gap-2 px-3 py-2 bg-card border border-border rounded-lg mb-2"
    >
      ...
      <span className={`text-sm ${isPaused ? 'text-warning' : 'text-foreground'}`}>
        {isPaused ? 'Paused' : 'Recording'} • <span className="font-mono tabular-nums">{formatDuration(displaySeconds)}</span>
      </span>
    </motion.div>
  );
};
