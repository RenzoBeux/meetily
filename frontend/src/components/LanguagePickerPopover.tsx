...
export function LanguagePickerPopover({
  ...
  return (
    <div
      ref={containerRef}
      className="w-72 rounded-lg bg-elevated border border-border shadow-glass overflow-hidden"
      role="dialog"
      ...
      <div className="max-h-80 overflow-y-auto py-1">
            ...
            {recentsResolved.map((opt) => (
              <button
                ...
                className={`flex w-full items-center justify-between px-3 py-1.5 text-sm hover:bg-accent text-left ${
                  value === opt.code ? "text-brand font-medium" : "text-foreground"
                }`}
              >
                <span>
                  {opt.label}{" "}
                ...
                {value === opt.code && <span className="text-brand" aria-hidden="true">âœ“</span>}
              </button>
            ))}
        ...
        {showAuto && (
          <button
            ...
            className={`flex w-full items-center justify-between px-3 py-1.5 text-sm hover:bg-accent text-left ${
              value === null ? "text-brand font-medium" : "text-foreground"
            }`}
          >
            ...
            {value === null && <span className="text-brand" aria-hidden="true">âœ“</span>}
          </button>
        )}
